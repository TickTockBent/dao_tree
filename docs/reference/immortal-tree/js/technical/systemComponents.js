var systemComponents = {
	...
	'overlay-head': {
		...
		<br>
		<template v-if="player.navTab === 'tree-tab-2'">
		<span class="overlayThing">You have </span>
		<h2 class="overlayThing" id="points">{{format(player.insights)}}</h2>
		<span class="overlayThing"> insights</span>
		<br>
		<span v-if="canGenPoints()" class="overlayThing">({{formatSmall(tmp.insightGen)}}/sec)</span>
		</template>
		<template v-else>
    ...
    'info-tab': {
        ...
        <br>
        The Modding Tree <a v-bind:href="'https://github.com/Acamaeda/The-Modding-Tree'" target="_blank" class="link" v-bind:style = "{'font-size': '14px', 'display': 'inline'}" >{{TMT_VERSION.tmtNum}}</a> by Acamaeda and FlamemasterNXF
        <br>
        The Prestige Tree made by Jacorb and Aarex
        <br><br>
        Inspired by <i>Renegade Immortal</i> (<span lang="zh">仙逆</span>) by Er Gen (<span lang="zh">耳根</span>). Unofficial fan work; not affiliated with the author or publishers.
		<br><br>
        ...
        <h3>Hotkeys</h3><br>
        <template v-if="modInfo.navHotkeys">
            <span v-for="line in modInfo.navHotkeys"><br>{{line}}</span>
            <br><br>
            <h3>Prestige hotkeys</h3><br>
        </template>
