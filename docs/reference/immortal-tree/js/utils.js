...
document.onkeydown = function (e) {
	...
	if (ctrlDown && e.shiftKey) key = "ctrl+shift+" + key
	else if (ctrlDown) key = "ctrl+" + key
	else if (e.shiftKey) key = "shift+" + key
	if (onFocused) return
	if (!ctrlDown && !e.shiftKey && typeof layerNavHotkey === "function" && LAYER_NAV_HOTKEYS && LAYER_NAV_HOTKEYS[key]) {
		e.preventDefault()
		layerNavHotkey(key)
		return
	}
	if ((ctrlDown || e.shiftKey) && hotkeys[key]) e.preventDefault()
	if (hotkeys[key]) {
		let k = hotkeys[key]
