...
function toDecimal(value, fallback = 0) {
	...
	if (value === undefined || value === null) return new Decimal(fallback)
	try { return new Decimal(value) }
	catch (e) { return new Decimal(fallback) }
}

const LAYER_DECIMAL_KEYS = ["points", "best", "total", "spentOnBuyables"]
const LAYER_EXTRA_DECIMALS = {
	sf: ["mortalInsights", "bestInsights"],
	asc: ["worldQi", "bestWorldQi", "totalWorldQi"],
}

function ensurePlayerDecimals() {
	if (!player) return
	player.points = toDecimal(player.points, modInfo.initialStartPoints ?? 0)
	if (player.insights !== undefined) player.insights = toDecimal(player.insights)
	if (player.bestInsights !== undefined) player.bestInsights = toDecimal(player.bestInsights)
	if (player.totalInsights !== undefined) player.totalInsights = toDecimal(player.totalInsights)
	for (const layer in layers) {
		if (!player[layer] || typeof player[layer] !== "object") continue
		const lr = player[layer]
		for (const key of LAYER_DECIMAL_KEYS) {
			if (lr[key] !== undefined) lr[key] = toDecimal(lr[key])
		}
		const extras = LAYER_EXTRA_DECIMALS[layer]
		if (extras) {
			for (const key of extras) {
				if (lr[key] === undefined) lr[key] = decimalZero
				else lr[key] = toDecimal(lr[key])
			}
		}
		if (lr.buyables && typeof lr.buyables === "object") {
			for (const id in lr.buyables) {
				lr.buyables[id] = toDecimal(lr.buyables[id])
			}
		}
	}
}
...
function load() {
	...
	if (get === null || get === undefined) {
		...
		if (typeof ensureOfflineProgress === "function") ensureOfflineProgress();
	}
	else {
	...
	changeTheme();
	changeTreeQuality();
	if (typeof screenshotMode === "function" && screenshotMode())
