...
function rowReset(row, layer) {
	for (lr in ROW_LAYERS[row]){
		if(layers[lr].doReset) {
			if (!isNaN(row)) {
				if (lr === "s" && player.s && player.s.activeChallenge && typeof clearSectChallengeSnapshot === "function") {
					clearSectChallengeSnapshot()
				}
				if (lr === "nsh" && player.nsh && player.nsh.activeChallenge && typeof clearNirvanaChallengeSnapshot === "function") {
					clearNirvanaChallengeSnapshot()
				}
...
function doReset(layer, force=false) {
	...
	if (typeof treeOf === "function" && treeOf(layer) === "cultivation") {
		player.points = (row == 0 ? decimalZero : getStartPoints())
		const rows = typeof CULTIVATION_ROWS !== "undefined" ? CULTIVATION_ROWS : [0, 1, 2, 3, 4]
		for (let i = 0; i < rows.length; i++) {
			const x = rows[i]
			if (x <= row) rowReset(x, layer)
		}
	} else if (typeof treeOf === "function" && treeOf(layer) === "sect") {
		const rows = typeof SECT_ROWS !== "undefined" ? SECT_ROWS : [10, 11, 12]
		for (let i = 0; i < rows.length; i++) {
			const x = rows[i]
			if (x <= row) rowReset(x, layer)
		}
...
function startChallenge(layer, x) {
	...
	if (enter && layer === "s") {
		player.s.challengeSnapshot = takeCultivationChallengeSnapshot()
		if (typeof applySectChallengeStart === "function" && sectChallengeUsesTrialReset(x)) {
			applySectChallengeStart(x)
		}
	}
	else if (enter && layer === "nsh") {
		player.nsh.challengeSnapshot = takeNirvanaChallengeSnapshot()
		if (typeof applyNirvanaBlightChallengeStart === "function" && nshChallengeUsesTrialReset(x)) {
			applyNirvanaBlightChallengeStart(x)
		}
	}
	else if ((enter || canExitChallenge(layer, x)) && layer !== "s" && layer !== "nsh") {
		doReset(layer, true)
	}
	if(enter) {
...
function completeChallenge(layer, x) {
	...
	if (!completions){
		Vue.set(player[layer], "activeChallenge", null)
		if (layer === "s" && typeof clearSectChallengeSnapshot === "function") clearSectChallengeSnapshot()
		if (layer === "nsh" && typeof clearNirvanaChallengeSnapshot === "function") clearNirvanaChallengeSnapshot()
...
function gameLoop(diff) {
	...
	addTime(diff)
	if (typeof onNirvanaTreeTab === "function" && onNirvanaTreeTab()) {
		if (typeof insightsEnsure === "function") insightsEnsure()
		const ig = typeof tmp.insightGen !== "undefined" ? tmp.insightGen : new Decimal(0)
		player.insights = player.insights.add(ig.times(diff)).max(0)
		if (player.insights.gt(player.bestInsights)) player.bestInsights = player.insights
		player.totalInsights = player.totalInsights.add(ig.times(diff))
	} else {
	...
	for (let x = maxRow; x >= 0; x--){
		for (item in TREE_LAYERS[x]) {
			...
			if (typeof artificerAutoLayers === "function" && artificerAutoLayers().includes(layer)) autobuyUpgrades(layer)
		}
	}
