...
**Architecture: runtime data-factory, not codegen.** TMT executes JS at load, so we don't generate files — we iterate the data table and construct layers at load. The layer functions TMT requires (`requires()`, `gainMult()`, milestone `done()`, etc.) are thin closures over the data row. **Hard rule: zero numeric literals in layer/factory code — every number resolves from a data row.** (The linter can assert this directly.)

```
// data/realms.ts — single source of truth
...
// build/layerFactory.ts — generic, parameterized, no magic numbers
REALM_DATA.forEach(r => addLayer(r.id, makeRealmLayer(r)))
// makeRealmLayer(r) → { row:r.row, requires(){return D(r.reqBase)},
//   milestones: makeMilestones(r.substages), unlocked(){return meets(r.unlock)}, … }
```

Meridian/temper buyables, the Body layer, and story-gate achievements (§8) are described by sibling data tables (`BODY_DATA`, `GATE_DATA`) consumed by the same pattern. The **linter** (§9) runs over all tables at load: no-dead-multiplier, completability, and "no numeric literal in generated code."

**Decision for sign-off:** runtime-factory (recommended — no build step, simplest, matches how TMT already loads) vs. codegen (only if we later need static analysis the runtime can't provide). Recommend factory for v0.1; revisit only if tooling demands it.
